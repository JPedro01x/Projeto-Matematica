...
export const localStore = {
  ...
  players: {
    ...
    insert: async (data: Partial<Player>) => {
      ...
      const playerId = data.id || nicknameToId(nickname);
      const nicknameAlreadyUsed = players.some(player =>
        player.id === playerId || normalizeNickname(player.nickname) === normalizeNickname(nickname)
      );

      if (nicknameAlreadyUsed) {
        return { data: null, error: new Error('Apelido já está em uso') };
      }
